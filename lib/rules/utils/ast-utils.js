...
const whileLoopPattern = /^(?:While)Statement$/u;
...
/**
 * Checks whether a given node is a while loop node or not.
 * The following types are while loop nodes:
 *
 * - WhileStatement
 * @param {ASTNode|null} node A node to check.
 * @returns {boolean} `true` if the node is a loop node.
...
function isWhileLoop(node) {
    return Boolean(node && whileLoopPattern.test(node.type));
}
